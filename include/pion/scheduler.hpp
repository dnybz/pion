...
public:
    ...
    inline static void sleep(uint32_t sleep_sec, uint32_t sleep_nsec) {
		auto wakeup_time = std::chrono::nanoseconds{ sleep_sec * 1000000000 + sleep_nsec };
        std::this_thread::sleep_for(wakeup_time);
    }
    ...
    {
		auto wakeup_time = std::chrono::nanoseconds{ sleep_sec * 1000000000 + sleep_nsec };
		wakeup_condition.wait_for(wakeup_lock, wakeup_time);
    }
    ...
    virtual void stop_threads(void) {
        if (! m_thread_pool.empty()) {
                 ...
                 i != m_thread_pool.end(); ++i)
            {
                ...
                if (i->get()->get_id() != current_thread.get_id()) (*i)->join();
            }
        }
    }
