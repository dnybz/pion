...
struct PION_API algorithm {
    ...
    static inline uint32_t to_uint32(unsigned char high, unsigned char mid1, unsigned char mid2, unsigned char low) {
        return (((uint32_t)high) << 24) | (((uint32_t)mid1) << 16) | (((uint32_t)mid2) << 8) | ((uint32_t)low);
    }
    ...
    static inline uint64_t to_uint64(unsigned char high, unsigned char mid1, unsigned char mid2, unsigned char mid3, unsigned char mid4, unsigned char mid5, unsigned char mid6, unsigned char low) {
        return (((uint64_t)high) << 56) | (((uint64_t)mid1) << 48) | (((uint64_t)mid2) << 40) | (((uint64_t)mid3) << 32)
            | (((uint64_t)mid4) << 24) | (((uint64_t)mid5) << 16) | (((uint64_t)mid6) << 8) | ((uint64_t)low);
    }
    ...
    /// convert sequence of three bytes to 24-bit unsigned integer
    template <typename Byte>
    static inline uint32_t to_uint24(const Byte *buf) {
    ...
    template <typename Byte>
    static inline void from_uint24(Byte *buf, const uint32_t n) {
        buf[0] = (n >> 16) & 0xFF;
        buf[1] = (n >> 8) & 0xFF;
        buf[2] = n & 0xFF;
    }
