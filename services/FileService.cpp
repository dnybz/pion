...
void FileService::set_option(const std::string& name, const std::string& value)
{
	...
	} else if (name == "max_cache_size") {
		m_max_cache_size = std::stoul(value);
...
void FileService::scanDirectory(const fs::path& dir_path)
{
...
void DiskFile::update(void)
{
    ...
    m_file_size = static_cast<std::streamsize>(fs::file_size( m_file_path ));
    m_last_modified = std::chrono::system_clock::to_time_t(fs::last_write_time( m_file_path ));
...
void DiskFile::read(void)
{
    ...
    if (!file_stream.is_open() || !file_stream.read((char*)m_file_content.get(), m_file_size)) {
        const std::string file_name = m_file_path.string();
		std::cout << "read_file_err: " << file_name << std::endl;
    }
}
...
{
    // get current values
    std::streamsize cur_size = static_cast<std::streamsize>(fs::file_size( m_file_path ));
    time_t cur_modified = std::chrono::system_clock::to_time_t(fs::last_write_time( m_file_path ));
...
void DiskFileSender::send(void)
{
    ...
    if (m_disk_file.hasFileContent()) {
        ...
        file_content_ptr = (char*)m_content_buf.get();

        // read a block of data from the file into the content buffer
        if (! m_file_stream.read((char*)m_content_buf.get(), m_file_bytes_to_send)) {
    ...
    if (m_bytes_sent + m_file_bytes_to_send >= m_disk_file.getFileSize()) {
        ...
        if (m_bytes_sent > 0) {
			...
			m_writer->send_final_chunk([self](const asio::error_code& write_error, std::size_t bytes_written ) {
				self->handle_write(write_error, bytes_written);
			});

        } else {
            // sending entire file at once
			auto self = shared_from_this();
			m_writer->send([self](const asio::error_code& write_error, std::size_t bytes_written) {
				self->handle_write(write_error, bytes_written);
			});
        }
    } else {
		...
		auto self = shared_from_this();
		m_writer->send_chunk([self](const asio::error_code& write_error, std::size_t bytes_written) {
			self->handle_write(write_error, bytes_written);
		});
    }
}
